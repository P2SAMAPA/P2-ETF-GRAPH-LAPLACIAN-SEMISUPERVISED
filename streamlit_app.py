...
st.set_page_config(page_title="Graph Laplacian Semi‑Supervised Learning", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Graph Laplacian Semi‑Supervised Learning</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">ETF correlation graph | Label propagation via Laplacian regularisation</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Graph Laplacian")
...
st.sidebar.markdown(f"**K‑NN neighbors:** {config.KNN_NEIGHBORS} | **α:** {config.ALPHA}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'graph_laplacian_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Positive probability: {etf['gl_score_norm']:.3f}</p>
...
with tab1:
    st.header("📊 Top ETFs by Label Propagation Positive Probability (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Graph Laplacian semi‑supervised learning** propagates labels through the ETF correlation graph.
        - Initial labels: ETFs with positive last‑day return = positive class (1); negative last‑day return = negative class (-1).
        - The graph is built using k‑nearest neighbours in correlation distance.
        - Label propagation solves `(I + α L) f = y` where `f` is the final scores, `y` the initial labels.
        - The score is the probability of being in the positive class after propagation.
...
st.sidebar.markdown("---")
st.sidebar.caption("Graph Laplacian Semi‑Supervised | Label propagation on ETF correlation graph")
